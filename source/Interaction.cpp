...
vector<int> Interaction::prefer_time_conv(vector<int> start_hour, vector<int> start_min, vector<int> end_hour, vector<int> end_min, vector<int> days)
{
	//default
	vector<int> rec;
	// Tong: Why its appending i instead of start_hour[i] into the returned variable rec?
	for (size_t i = 0; i < start_hour.size(); ++i) rec.push_back(i);
	return rec;
}

// this function is used to manually add class_rooms in the configuration of course schedule requirements
...
// rec = sub_count, tp = type
bool Interaction::check_comp(int rec, int tp)
{
	if (tp == 1) return true;  // TIME block
	if (tp == 2 && rec == 4) return true; // INSTRUCTOR block
	if (tp == 3 && rec == 7) return true; // COURSE block
	if (tp == 4 && rec == 6) return true; // CLASS block
	if (tp == 5 && rec == 5) return true; // CLASSROOM block
	return false;
}
...
void Interaction::init_from_text_file_v2(string _name)
{
	...
	{
		string _s = regulate(s); // delete white spaces in s to get _s
		line_rec++;
